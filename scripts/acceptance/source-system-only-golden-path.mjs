...
const tenantId = process.env.SOURCE_SYSTEM_ONLY_TENANT_ID || `tenant-source-system-only-${Date.now()}`;
const sourceSystem = normalize(process.env.SOURCE_SYSTEM_ONLY_SOURCE_SYSTEM || 'ERP');
const agentId = process.env.SOURCE_SYSTEM_ONLY_AGENT_ID || `agent-source-system-only-triage-${Date.now()}`;
const poolId = process.env.SOURCE_SYSTEM_ONLY_POOL_ID || `pool-source-system-only-${slug(sourceSystem)}-triage`;
const poolCode = process.env.SOURCE_SYSTEM_ONLY_POOL_CODE || `${sourceSystem}_TRIAGE_POOL`;
const flowId = process.env.SOURCE_SYSTEM_ONLY_FLOW_ID || `flow-source-system-only-${slug(sourceSystem)}`;
const operatorId = process.env.SOURCE_SYSTEM_ONLY_OPERATOR_ID || 'source-system-only-release-gate';
const gatewayUrl = process.env.SOURCE_SYSTEM_ONLY_GATEWAY_URL || process.env.NETTY_URL || 'http://127.0.0.1:18081';
const credentialToken = process.env.SOURCE_SYSTEM_ONLY_AGENT_TOKEN || `source-system-only-token-${Date.now()}`;
const pollTimeoutMs = intEnv('SOURCE_SYSTEM_ONLY_POLL_TIMEOUT_MS', 60000);
const pollIntervalMs = intEnv('SOURCE_SYSTEM_ONLY_POLL_INTERVAL_MS', 1500);
const dryRun = hasArg('--dry-run') || flag('SOURCE_SYSTEM_ONLY_DRY_RUN');
const negative = hasArg('--negative') || flag('SOURCE_SYSTEM_ONLY_NEGATIVE');
const skipAgentSetup = hasArg('--skip-agent-setup') || flag('SOURCE_SYSTEM_ONLY_SKIP_AGENT_SETUP');
const skipRuntimeAssertion = hasArg('--skip-runtime-assertion') || flag('SOURCE_SYSTEM_ONLY_SKIP_RUNTIME_ASSERTION');
...
function sourceSystemPayload() {
  return {
    ...
    displayName: `${sourceSystem} SourceSystem-only Source System`,
    description: 'SourceSystem-only Golden Path: SourceSystem-only intake must fall back to default TRIAGE_POOL.',
    status: 'ACTIVE',
  };
}
...
function agentSetupPayload() {
  return {
    tenantId,
    agentId,
    agentName: `${sourceSystem} SourceSystem-only Triage Agent`,
...
function agentPoolPayload(status = 'ACTIVE') {
  return {
    ...
    status,
    description: 'Default TRIAGE_POOL for SourceSystem-only golden path.',
    members: [{
      tenantId,
      poolId,
      poolCode,
      ...
      priority: 10,
      weight: 1,
      metadata: { sourceSystemOnly: true },
    }],
    metadata: {
      sourceSystemOnly: true,
...
function sourceFlowPayload(status = 'ACTIVE', defaultPool = poolId, withRule = false) {
  const rules = withRule ? [{
    ...
    priority: 10,
    enabled: true,
    condition: { sourceSystemOnlyKnownEventOverride: true },
  }] : [];
  return {
    tenantId,
    flowId,
    flowCode: `${sourceSystem}_SOURCE_FLOW_SOURCE_SYSTEM_ONLY`,
    flowName: `${sourceSystem} Source Flow SourceSystem-only`,
    sourceSystem,
    flowType: 'SOURCE_FLOW',
    defaultPoolId: defaultPool,
    status,
    description: 'SourceSystem-only Source Flow: unknown events must route to default TRIAGE_POOL.',
...
function sourceSystemOnlyIntakePayload() {
  ...
  return {
    ...
    sourceSystem,
    severity: 'CRITICAL',
    message: `${sourceSystem} SourceSystem-only SourceSystem-only unknown event ${uniqueSuffix()}`,
    attributes: {
      sourceSystemOnly: true,
      contract: SOURCE_SYSTEM_ONLY_INTAKE_CONTRACT,
      rawCode: 'SOURCE_SYSTEM_ONLY_RAW_UNKNOWN',
...
async function submitSourceSystemOnlyIntake() {
  ...
  assert(!Object.prototype.hasOwnProperty.call(payload, 'eventType'), 'SourceSystem-only intake payload must omit eventType');
  assert(!Object.prototype.hasOwnProperty.call(payload, 'objectType'), 'SourceSystem-only intake payload must omit objectType');
  assert(!Object.prototype.hasOwnProperty.call(payload, 'errorCode'), 'SourceSystem-only intake payload must omit errorCode');
  return request('SourceSystem-only /api/events/intake', 'POST', '/api/events/intake', payload, {
    dryRunResponse: {
      taskCreated: true,
      taskId: `task-source-system-only-${uniqueSuffix()}`,
      taskType: 'TRIAGE',
      assignmentCreated: true,
      assignmentId: `assignment-source-system-only-${uniqueSuffix()}`,
      selectedAgentId: agentId,
      routingDecisionId: `routing-source-system-only-${uniqueSuffix()}`,
      eventStage: 'EXTERNAL',
      sourceSystem,
      correlationId: `corr-source-system-only-${uniqueSuffix()}`,
    },
  });
}
...
async function runPositiveGoldenPath() {
  console.log(`[SourceSystem-only] Positive golden path: ${tenantId} / ${sourceSystem} / ${poolCode} / ${agentId}`);
  ...
  console.log('[SourceSystem-only] PASS: SourceSystem-only intake routed to default TRIAGE_POOL and selected a Pool member Agent without Capability gate.');
}

async function runNegativePoolFirstBlockerCases() {
  console.log('[SourceSystem-only] Negative blocker plan: verifying gate covers Pool-first failure vocabulary.');
  const cases = [
  ...
  for (const testCase of cases) {
    console.log(`[SourceSystem-only][NEGATIVE] ${testCase.code}: ${testCase.fix}`);
    if (!dryRun) {
...
async function main() {
  console.log(`[SourceSystem-only] ${SOURCE_SYSTEM_ONLY_INTAKE_CONTRACT}`);
  console.log(`[SourceSystem-only] ${NO_CAPABILITY_ROUTING_GATE_CONTRACT}`);
  await runPositiveGoldenPath();
  if (negative) await runNegativePoolFirstBlockerCases();
}

main().catch((error) => {
  console.error(`[SourceSystem-only] FAIL: ${error.stack || error.message}`);
  process.exit(1);
});
