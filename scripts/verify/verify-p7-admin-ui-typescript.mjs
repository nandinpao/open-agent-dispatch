...
const files = [
  ...
  'ai-event-gateway-admin-ui/lib/dispatch-governance/labels.ts',
  ...
  ...fs.readdirSync(path.join(ROOT, 'ai-event-gateway-admin-ui/components/dispatch-governance'))
    .filter((name) => name.endsWith('.tsx'))
    .map((name) => `ai-event-gateway-admin-ui/components/dispatch-governance/${name}`),
];
...
console.log(`[PASS] P7 TypeScript/TSX syntax transpile (${files.length} files).`);
