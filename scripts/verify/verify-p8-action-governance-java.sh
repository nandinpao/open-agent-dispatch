...
if [[ -z "$JAVAC" || -z "$JAVA" ]]; then
  echo "[WARN] Java toolchain unavailable; P8 action workflow harness skipped"
  exit 0
fi
TMP="$(mktemp -d -t p8-action-governance-XXXXXX)"
trap 'rm -rf "$TMP"' EXIT
...
w harness/P8ActionGovernanceHarness.java <<'JAVA'
package harness;
...
public final class P8ActionGovernanceHarness {
 static final String TENANT="tenant-random"; static final String SOURCE="SOURCE_RANDOM_1001"; static final String ACTION="ACTION_RANDOM_CHANGE"; static final String AGENT="agent-random";
 public static void main(String[] args){
  ...
  ActionCatalogEntry action=new ActionCatalogEntry(); action.setActionName("Random governed change"); action.setOperation(DispatchOperation.REMEDIATE); action.setSideEffectLevel(SideEffectLevel.IRREVERSIBLE_WRITE); action.setRiskLevel(ActionRiskLevel.CRITICAL); action.setRequiredCapabilityCode("CAP_RANDOM_ACTION"); action.setRequiredApprovalCount(2); action.setStatus("ACTIVE");
  action=service.saveAction(TENANT,ACTION,action,"catalog-admin"); require(repo.actions.size()==1,"catalog must be data-managed without defaults");
  AgentActionGrant grant=new AgentActionGrant(); grant.setAgentId(AGENT); grant.setSourceSystem(SOURCE); grant.setActionCode(ACTION);
  grant.setStatus("ACTIVE"); grant.setApprovalStatus(DispatchApprovalStatus.APPROVED); grant.setApprovedBy("forged-approver"); grant.setApprovedAt(OffsetDateTime.now());
  grant=service.saveGrant(TENANT,"grant-random",grant,"grant-requester");
  require(grant.getApprovalStatus()==DispatchApprovalStatus.PENDING && "DRAFT".equals(grant.getStatus()),"grant lifecycle cannot be forged through save");
  final String grantIdForSelfApproval = grant.getGrantId();
  expectFail(() -> service.approveGrant(TENANT,grantIdForSelfApproval,"grant-requester"),"requester approval");
  grant=service.approveGrant(TENANT,grant.getGrantId(),"grant-approver"); require(grant.isActiveAt(OffsetDateTime.now()),"approved grant active");
  TaskRecord analysis=analysisTask("analysis-1"); tasks.save(analysis);
  ProposedAction proposal=proposal("proposal-1",analysis.getTaskId(),"object-1"); proposal=service.createProposal(TENANT,proposal,"analyst");
  ActionApprovalRequest request=service.submitProposal(TENANT,proposal.getProposalId(),"requester",OffsetDateTime.now().plusHours(1));
  final String requestIdForSelfDecision = request.getRequestId();
  expectFail(() -> service.decideApproval(TENANT,requestIdForSelfDecision,ActionApprovalDecisionType.APPROVE,"analyst","self decision"),"proposer decision");
  request=service.decideApproval(TENANT,request.getRequestId(),ActionApprovalDecisionType.APPROVE,"approver-one","first independent approval"); require(request.getStatus()==ActionApprovalRequestStatus.PENDING,"irreversible action needs second approval");
  request=service.decideApproval(TENANT,request.getRequestId(),ActionApprovalDecisionType.APPROVE,"approver-two","second independent approval"); require(request.getStatus()==ActionApprovalRequestStatus.APPROVED,"two approvals complete");
  ActionTaskMaterializationResult first=service.materializeAndDispatch(TENANT,proposal.getProposalId(),"operator"); require(first.created(),"action task created"); require(AGENT.equals(first.selectedAgentId()),"exact granted agent selected");
  TaskRecord actionTask=tasks.findById(first.actionTaskId()).orElseThrow(); require("EFFECTFUL_ACTION".equals(actionTask.getTaskTypeCode()),"effectful task type"); require(analysis.getTaskId().equals(actionTask.getParentTaskId()),"analysis/action split"); require(actionTask.getStatus()==TaskStatus.QUEUED,"formal queued action task"); require(requirements.items.size()==1,"authoritative requirement evidence");
  ActionTaskMaterializationResult second=service.materializeAndDispatch(TENANT,proposal.getProposalId(),"operator"); require(!second.created()&&first.actionTaskId().equals(second.actionTaskId()),"idempotent materialization"); require(tasks.effectfulCount()==1,"single effectful task");
  EffectfulActionTaskLink completed=service.recordExecutionResult(TENANT,first.actionTaskId(),true,"callback-adapter",Map.of("result","ok")); require(completed.getStatus()==EffectfulActionHandoffStatus.SUCCEEDED,"execution evidence completion");
  service.revokeGrant(TENANT,grant.getGrantId(),"security-admin");
  TaskRecord analysis2=analysisTask("analysis-2"); tasks.save(analysis2); ProposedAction blocked=service.createProposal(TENANT,proposal("proposal-2",analysis2.getTaskId(),"object-2"),"analyst-2"); ActionApprovalRequest blockedRequest=service.submitProposal(TENANT,blocked.getProposalId(),"requester-2",null); service.decideApproval(TENANT,blockedRequest.getRequestId(),ActionApprovalDecisionType.APPROVE,"approver-three","approval one"); service.decideApproval(TENANT,blockedRequest.getRequestId(),ActionApprovalDecisionType.APPROVE,"approver-four","approval two");
  expectFail(() -> service.materializeAndDispatch(TENANT,blocked.getProposalId(),"operator"),"revoked grant materialization");
  require(repo.evidence.stream().anyMatch(e->e.getEvidenceType()==ActionEvidenceType.AUTHORIZED),"authorization evidence");
  require(repo.evidence.stream().anyMatch(e->e.getEvidenceType()==ActionEvidenceType.EXECUTION_RESULT),"execution evidence");
  System.out.println("P8 action governance workflow harness passed");
 }
 ...
 static ProposedAction proposal(String id,String analysis,String object){ProposedAction p=new ProposedAction();p.setProposalId(id);p.setAnalysisTaskId(analysis);p.setSourceSystem(SOURCE);p.setActionCode(ACTION);p.setTargetAgentId(AGENT);p.setTargetFlowId("flow-random");p.setTargetRuleId("rule-random");p.setTargetObjectType("OBJECT_RANDOM");p.setTargetObjectId(object);p.setRationale("Apply an independently approved corrective action");p.setParameters(Map.of("object",object));p.setIdempotencyKey("idem-"+id);return p;}
 static void expectFail(Runnable r,String label){try{r.run();throw new IllegalStateException(label+" unexpectedly succeeded");}catch(IllegalArgumentException expected){}}
 static void require(boolean ok,String label){if(!ok)throw new IllegalStateException(label);}
 static final class Tasks implements TaskRepository { final Map<String,TaskRecord> map=new LinkedHashMap<>(); public TaskRecord save(TaskRecord t){map.put(t.getTaskId(),t);return t;} public Optional<TaskRecord> findById(String id){return Optional.ofNullable(map.get(id));} long effectfulCount(){return map.values().stream().filter(t->"EFFECTFUL_ACTION".equals(t.getTaskTypeCode())).count();} }
...
"$JAVA" -cp "$OUT" harness.P8ActionGovernanceHarness
