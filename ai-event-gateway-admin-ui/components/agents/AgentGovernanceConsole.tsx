...
export function AgentGovernanceConsole() {
  ...
  async function reconcileBlockedRuntimes() {
    ...
    setError(null);
    try {
      const result = await coreAdminApi.reconcileBlockedAgentRuntimes({ limit: 500 });
      setReconcileResult(result);
    } catch (caught) {
      setError(caught instanceof Error ? caught.message : 'Refresh Agent Runtime session.');
    } finally {
      setBusy(false);
    }
  }
    ...
    <div className="space-y-5">
      <div className="rounded-2xl border border-blue-100 bg-blue-50 p-4 text-sm text-blue-900">
        <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
          <div>
            <div className="font-semibold">Agent Actions</div>
            <div className="mt-1">
              Create and approve Agent profiles, review credentials and runtime connectivity, configure Dispatch Access, and verify Dispatch Flow usage. Runtime telemetry never replaces Core dispatch authority.
            </div>
          </div>
          <button type="button" onClick={reconcileBlockedRuntimes} disabled={busy} className="rounded-xl border border-blue-300 bg-white px-3 py-2 text-sm font-bold text-blue-700 hover:bg-blue-50 disabled:opacity-50">
            {busy ? 'Refreshing…' : 'Refresh Runtime Status'}
          </button>
        </div>
