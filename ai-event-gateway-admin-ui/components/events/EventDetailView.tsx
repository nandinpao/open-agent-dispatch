    ...
    <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500"> SELF Gateway local event stream rawevent and routing Result cluster-wide event aggregation.</p>
        </div>
...
function RoutingDecisionPanel({ event }: Readonly<{ event: GatewayEventDetail }>) {
  ...
  return (
    <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
      ...
      <p className="mt-1 text-sm text-slate-500"> Gateway / Core Event details Agent / Node, and display advanced policy compatibility diagnostics.</p>
      <div className="mt-4 grid gap-3 md:grid-cols-4">
      ...
      <div className="mt-4 rounded-xl bg-slate-50 p-4 text-sm text-slate-700">
        {decision?.reason ?? 'No routing decision reason is available.'}
      </div>
    </section>
  );
}
...
function RelatedTaskPanel({ event }: Readonly<{ event: GatewayEventDetail }>) {
  ...
  return (
    <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
      ...
      <p className="mt-1 text-sm text-slate-500">Event details Task list.</p>
      {relatedTasks.length === 0 ? (
        <EmptyState title="No Task has been created" description="Event details RECEIVED Status, or routing Failed." />
      ) : (
...
function EventPayloadPanel({ event }: Readonly<{ event: GatewayEventDetail }>) {
  return (
    <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
      ...
      <p className="mt-1 text-sm text-slate-500"> Gateway raw JSON Task request payload </p>
      <div className="mt-4">
        {event.payload ? <JsonViewer value={event.payload} /> : <EmptyState title="No payload" description="The backend did not return an event payload." />}
      </div>
    </section>
  );
}
...
export function EventDetailView({ eventId }: Readonly<{ eventId: string }>) {
  ...
  if (loading) return <LoadingBox label={`Loading event ${eventId}…`} />;
  if (error) return <ErrorBox message={error} />;
  if (!data) return <EmptyState title="Event not found" description="No data is currently available. Event detail." />;
