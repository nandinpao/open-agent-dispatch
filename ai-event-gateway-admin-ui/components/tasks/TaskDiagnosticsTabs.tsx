    ...
    <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
      <div className="flex flex-col gap-2 lg:flex-row lg:items-end lg:justify-between">
        <div>
          ...
          <h2 className="mt-1 text-lg font-black text-slate-950">Timeline Event Log </h2>
          <p className="mt-1 text-sm leading-6 text-slate-500"> Lifecycle Stepper lifecycle,runtime,recovery,raw diagnostics.</p>
        </div>
      </div>
      <div className="mt-4 flex flex-wrap gap-2 border-b border-slate-200 pb-3">
        {tabs.map((tab) => {
          ...
          return (
            ...
            >
              {tab.label}
              {tab.badge ? <span className={`ml-2 rounded-full px-2 py-0.5 text-xs ${selectedTab ? 'bg-white/20 text-white' : 'bg-white text-slate-500'}`}>{tab.badge}</span> : null}
            </button>
          );
        })}
      </div>
