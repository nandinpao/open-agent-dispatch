...
DISPATCH_REQUEST_ID=$(curl -sS "${BASE_URL}/api/dispatch-requests/status/APPROVED?limit=1" | python3 -c 'import json,sys; data=json.load(sys.stdin); print(data[0]["dispatchRequestId"] if data else "")')
if [ -z "${DISPATCH_REQUEST_ID}" ]; then
  DISPATCH_REQUEST_ID=$(curl -sS "${BASE_URL}/api/dispatch-requests/status/DISPATCHED?limit=1" | python3 -c 'import json,sys; data=json.load(sys.stdin); print(data[0]["dispatchRequestId"] if data else "")')
fi
if [ -z "${DISPATCH_REQUEST_ID}" ]; then
...
$(python3 - <<'PY'
import json, os
r=json.loads(os.environ['DISPATCH_JSON'])
...
import json, os
print(json.loads(os.environ['DISPATCH_JSON']).get('attemptCount',1))
PY
)
fi
