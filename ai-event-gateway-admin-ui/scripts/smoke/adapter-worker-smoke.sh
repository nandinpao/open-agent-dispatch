...
curl -fsS "${BASE_URL}/api/core/status" | jq '{version, adapterExecutorMode, adapterExecutorExternalMode}'

echo "[2/5] Adapter action metadata"
curl -fsS "${BASE_URL}/api/adapter-actions/metadata" | jq '{executorMode, executorExternalMode, store}'
...
echo "${CLAIM_RESPONSE}" | jq '{actionId, adapterType, actionType, status, claimedBy, leaseExpiresAt}'
ACTION_ID=$(echo "${CLAIM_RESPONSE}" | jq -r '.actionId')
...
curl -fsS -X POST "${BASE_URL}/internal/adapter-actions/${ACTION_ID}/heartbeat" \
  ...
  | jq '{actionId, status, claimedBy, workerHeartbeatAt, leaseExpiresAt}'
...
curl -fsS -X POST "${BASE_URL}/internal/adapter-actions/${ACTION_ID}/complete" \
  ...
  | jq '{actionId, status, responseRef, claimedBy, leaseExpiresAt}'
