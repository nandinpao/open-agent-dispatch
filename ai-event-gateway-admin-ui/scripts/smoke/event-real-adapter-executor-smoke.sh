...
import json
try:
    print(json.load(open('/tmp/aeg-core-p10-event-response.json')).get('taskId') or '')
except Exception:
    print('')
PY
)
...
import json
try:
    print(json.load(open('/tmp/aeg-core-p10-event-response.json')).get('dispatchRequestId') or '')
except Exception:
    print('')
PY
)
