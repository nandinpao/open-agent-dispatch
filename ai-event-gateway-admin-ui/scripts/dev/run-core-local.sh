...
require_file "$ROOT_DIR/ai-event-gateway-core-app/pom.xml"
...
if [[ -d "$HOME/.m2/repository/com/opensocket/ai-event-gateway-core-parent" ]]; then
  echo "[INFO] Removing cached ai-event-gateway-core-parent from ~/.m2 to avoid stale failed parent resolution."
  rm -rf "$HOME/.m2/repository/com/opensocket/ai-event-gateway-core-parent"
fi

# Install required reactor artifacts before invoking Flyway from the app module.
# Flyway Maven plugin is executed from ai-event-gateway-core-app/pom.xml only; without
...
cd "$ROOT_DIR"
exec mvn -U -f "$ROOT_DIR/pom.xml" -pl ai-event-gateway-core-app -am spring-boot:run -Dspring-boot.run.profiles=local "$@"
