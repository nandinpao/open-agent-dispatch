...
def production_source_roots(root: Path) -> list[Path]:
    roots: list[Path] = []
    for module in sorted(root.glob("ai-event-gateway-core-*")):
        source_root = module / "src/main/java/com/opensocket/aievent/core"
        if source_root.is_dir():
            roots.append(source_root)
    database_root = root / "ai-event-gateway-database-platform/src/main/java/com/opensocket/aievent/database"
