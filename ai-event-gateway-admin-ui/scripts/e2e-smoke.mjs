...
const defaultCoreOrigin = publicHost ? `${publicScheme}://${publicHost}:${process.env.CORE_HTTP_PORT || 18080}` : 'http://localhost:18080';
const defaultNettyOrigin = publicHost ? `${publicScheme}://${publicHost}:${process.env.NETTY_ADMIN_HTTP_PORT || 18081}` : 'http://localhost:18081';
const adminOrigin = normalizeOrigin(process.env.ADMIN_UI_ORIGIN || defaultAdminOrigin);
const coreOrigin = normalizeOrigin(process.env.CORE_BACKEND_ORIGIN || defaultCoreOrigin);
const nettyOrigin = normalizeOrigin(process.env.NETTY_BACKEND_ORIGIN || defaultNettyOrigin);
