...
for profile in local dev prod; do
  ...
  grep -q "store: \${EVENT_DEDUP_STORE:REDISSON}" "${yml}"
...
# Env examples must use SharedUtility pg.single variables and default dedup to REDISSON in local/dev/prod.
for env in local dev prod; do
  ...
  grep -q "EVENT_DEDUP_STORE=REDISSON" "${f}"
...
if [[ -f "${DB_CONFIG}" ]]; then
  ...
  grep -q '<artifactId>redisson-client</artifactId>' "${ROOT_DIR}/ai-event-gateway-core-app/pom.xml"
