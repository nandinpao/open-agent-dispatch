...
grep -q -- "-pl ai-event-gateway-core-app -am" "$ROOT_DIR/scripts/db/_maven_reactor.sh" || fail "_maven_reactor.sh must install the app reactor closure with -pl ai-event-gateway-core-app -am."
...
for f in flyway-migrate-postgres.sh flyway-validate-postgres.sh flyway-info-postgres.sh flyway-repair-postgres.sh; do
  ...
  grep -q 'ai-event-gateway-core-app/pom.xml' "$path" || fail "$f must keep Flyway goal scoped to the app module pom."
done
