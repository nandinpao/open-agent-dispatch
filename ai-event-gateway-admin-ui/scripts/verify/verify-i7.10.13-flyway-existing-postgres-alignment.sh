...
grep -q 'flyway-maven-plugin' ai-event-gateway-core-app/pom.xml || fail "Core app POM must configure flyway-maven-plugin"
grep -q 'flyway-database-postgresql' ai-event-gateway-core-app/pom.xml || fail "Flyway Maven plugin must include PostgreSQL database support"
...
grep -q 'baseline-on-migrate: ${FLYWAY_BASELINE_ON_MIGRATE:false}' ai-event-gateway-core-app/src/main/resources/application-local.yml || fail "local Flyway baseline-on-migrate must default false"
grep -q 'baseline-on-migrate: ${FLYWAY_BASELINE_ON_MIGRATE:false}' ai-event-gateway-core-app/src/main/resources/application-dev.yml || fail "dev Flyway baseline-on-migrate must default false"
grep -q 'baseline-on-migrate: ${FLYWAY_BASELINE_ON_MIGRATE:false}' ai-event-gateway-core-app/src/main/resources/application-prod.yml || fail "prod Flyway baseline-on-migrate must default false"
