...
test -d ai-event-gateway-core-observability/src/main/java/com/opensocket/aievent/core/observability
test -f ai-event-gateway-core-observability/src/main/java/com/opensocket/aievent/core/observability/CoreObservabilityModule.java
test -f ai-event-gateway-core-observability/src/main/java/com/opensocket/aievent/core/observability/CoreMetricsService.java
test -f ai-event-gateway-core-observability/src/main/java/com/opensocket/aievent/core/observability/CoreOperationalHealthIndicator.java
test ! -d ai-event-gateway-core-app/src/main/java/com/opensocket/aievent/core/observability

test -f ai-event-gateway-core-incident/src/main/java/com/opensocket/aievent/core/incident/IncidentOperationalQuery.java
test -f ai-event-gateway-core-agent-control/src/main/java/com/opensocket/aievent/core/agent/AgentControlOperationalQuery.java
test -f ai-event-gateway-core-task-orchestration/src/main/java/com/opensocket/aievent/core/task/TaskOperationalQuery.java
test -f ai-event-gateway-core-execution-control/src/main/java/com/opensocket/aievent/core/dispatch/ExecutionOperationalQuery.java
test -f ai-event-gateway-core-event-processing/src/main/java/com/opensocket/aievent/core/processing/EventProcessingOperationalQuery.java
...
if grep -R --include='*.java' -nE '^import .*\.(Repository|Dao);' \
    ai-event-gateway-core-*/src/main/java/com/opensocket/aievent/core/api \
    ai-event-gateway-core-observability/src/main/java 2>/dev/null; then
...
root=Path('.')
modules=sorted(p for p in root.glob('ai-event-gateway-core-*') if (p/'pom.xml').is_file()) + [root/'ai-event-gateway-database-platform']
source_to_module={}
type_to_source={}
type_to_module={}
...
for source, source_module in source_to_module.items():
    ...
    for imported in import_re.findall(text):
        ...
        if (source_module == 'ai-event-gateway-database-platform'
...
for module in modules:
    ...
    for dep in tree.findall('.//m:dependencies/m:dependency',ns):
        ...
        if artifact is not None and artifact.text and (artifact.text.startswith('ai-event-gateway-core-') or artifact.text == 'ai-event-gateway-database-platform'):
