...
KERNEL="ai-event-gateway-core-kernel/src/main/java/com/opensocket/aievent/core/kernel/persistence"
DISPATCH="ai-event-gateway-core-execution-control/src/main/java/com/opensocket/aievent/core/dispatch"
DB="ai-event-gateway-database-platform"
...
for module in \
  ...
  ai-event-gateway-database-platform; do
  grep -q '<artifactId>ai-event-gateway-core-kernel</artifactId>' "$module/pom.xml"
done
...
grep -q 'properties.getClaimLease(),[[:space:]]*$' \
  ai-event-gateway-core-domain-events/src/main/java/com/opensocket/aievent/core/outbox/OutboxEventDispatcher.java
grep -q 'batch.getFirst()' \
  ai-event-gateway-core-domain-events/src/main/java/com/opensocket/aievent/core/outbox/OutboxEventDispatcher.java
grep -q 'batch.getFirst()' \
  ai-event-gateway-core-integration-events/src/main/java/com/opensocket/aievent/core/integration/IntegrationEventDeliveryService.java
...
test -f ai-event-gateway-core-app/src/test/java/com/opensocket/aievent/core/container/PostgresClaimLeaseConcurrencyContainerTest.java
grep -q 'concurrentOutboxWorkersMustNotClaimTheSameRows' \
  ai-event-gateway-core-app/src/test/java/com/opensocket/aievent/core/container/PostgresClaimLeaseConcurrencyContainerTest.java
...
for type in ClaimOwnership ClaimRequest LeaseRenewalRequest PersistenceWriteResult; do
  grep -q "com.opensocket.aievent.core.kernel.persistence.${type},persistence-contract" \
