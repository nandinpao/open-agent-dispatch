...
#
# Why this exists:
# Running the Flyway Maven plugin from control-plane-app/pom.xml alone
...
ensure_core_reactor_root() {
  ...
  grep -q "<artifactId>control-plane-parent</artifactId>" "${root_dir}/pom.xml" \
    || maven_fail "${root_dir}/pom.xml is not control-plane-parent. Run from the full Core reactor root."
  [[ -f "${root_dir}/control-plane-app/pom.xml" ]] \
    || maven_fail "Missing control-plane-app/pom.xml under ${root_dir}. Re-extract the full Core zip."
}
