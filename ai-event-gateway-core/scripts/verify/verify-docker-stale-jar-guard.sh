...
[[ -f control-plane-app/Dockerfile ]] || fail "Dockerfile missing"

grep -q "EXPECTED_SHARED_UTILITY_VERSION=${EXPECTED_VERSION}" control-plane-app/Dockerfile \
  || fail "Dockerfile must define EXPECTED_SHARED_UTILITY_VERSION=${EXPECTED_VERSION}"

grep -q "database-\${EXPECTED_SHARED_UTILITY_VERSION}.jar" control-plane-app/Dockerfile \
  || fail "Dockerfile must verify exact database artifact version"

grep -q "redisson-client-\${EXPECTED_SHARED_UTILITY_VERSION}.jar" control-plane-app/Dockerfile \
  || fail "Dockerfile must verify exact redisson-client artifact version"

grep -Fq "database|redisson-client)-1\.0-SNAPSHOT" control-plane-app/Dockerfile \
...
grep -q "dockerfile: control-plane-app/Dockerfile" deploy/docker/docker-compose.core-local.yml \
