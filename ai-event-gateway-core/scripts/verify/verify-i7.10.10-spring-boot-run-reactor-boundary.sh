...
APP_POM="$ROOT_DIR/control-plane-app/pom.xml"
...
grep -q '<artifactId>control-plane-parent</artifactId>' "$ROOT_POM" || fail "Root pom is not the Core parent"
...
grep -q -- '-pl control-plane-app -am spring-boot:run' "$RUN_SCRIPT" || fail "run-core-local.sh must run spring-boot:run against the app module with -am"

# Prevent a future regression where the parent skip guard is removed while keeping docs/scripts unchanged.
if grep -R -n '<skip>false</skip>' "$ROOT_DIR" --include='pom.xml' | grep -v 'control-plane-app/pom.xml' >/dev/null; then
  fail "Only control-plane-app may enable spring-boot:run with skip=false"
fi
