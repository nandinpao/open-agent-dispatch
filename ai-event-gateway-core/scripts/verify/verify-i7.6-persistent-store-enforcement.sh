...
if grep -R "matchIfMissing = true" \
  ...
  agent-control/src/main/java \
  domain-events/src/main/java \
  event-processing/src/main/java \
  execution-control/src/main/java \
  incident/src/main/java \
  integration-events/src/main/java \
  task-orchestration/src/main/java \
...
grep -q 'event.dedup.store", "MYBATIS"' "${VALIDATOR}" || { echo "ERROR: validator must require MYBATIS dedup source of truth in prod" >&2; exit 1; }
