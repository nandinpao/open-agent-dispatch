...
test -f database-platform/src/main/resources/db/migration/V19__integration_event_outbox.sql

grep -q '^integration_event_outbox,integration-events,' architecture/table-ownership.csv
grep -q '^adapter-worker-app,worker,' architecture/deployable-boundaries.csv
...
grep -q '<artifactId>service-contracts</artifactId>' "$WORKER/pom.xml"
if grep -Eq '(adapter-action|domain-events|incident|task-orchestration|execution-control|database-platform|ai-event-gateway-core-db-support|ai-event-gateway-core-db-migration)' "$WORKER/pom.xml"; then
  echo "Adapter worker must not depend on Core domain or persistence modules" >&2
  exit 1
fi

# M8 intentionally has two executable Spring Boot applications plus the parent plugin guard.
test "$(grep -R -l --include='pom.xml' '<artifactId>spring-boot-maven-plugin</artifactId>' . | wc -l | tr -d ' ')" = "3"
grep -q '<artifactId>spring-boot-maven-plugin</artifactId>' control-plane-app/pom.xml
...
grep -Eq 'Map<String,[[:space:]]*List<ModuleEventHandler' domain-events/src/main/java/com/opensocket/aievent/core/outbox/OutboxEventDispatcher.java
if grep -q 'Duplicate module event handler' domain-events/src/main/java/com/opensocket/aievent/core/outbox/OutboxEventDispatcher.java; then
  echo "M8 dispatcher must permit multiple handlers per event type" >&2
  exit 1
fi

grep -q 'IntegrationEventProperties.class' control-plane-app/src/main/java/com/opensocket/aievent/core/AiEventGatewayCoreApplication.java
grep -q 'integration-events:' control-plane-app/src/main/resources/application.yml
grep -q 'adapter-executor:' control-plane-app/src/main/resources/application-hybrid-worker.yml
