...
grep -q '<module>database-platform</module>' pom.xml
grep -q '<artifactId>database-platform</artifactId>' control-plane-app/pom.xml
! grep -q '<artifactId>ai-event-gateway-core-db-support</artifactId>' control-plane-app/pom.xml
! grep -q '<artifactId>ai-event-gateway-core-db-migration</artifactId>' control-plane-app/pom.xml

test -f control-plane-app/src/test/java/com/opensocket/aievent/core/architecture/P0DatabasePlatformStructureTest.java
