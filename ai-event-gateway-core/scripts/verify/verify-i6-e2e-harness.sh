...
if [ -z "$CORE_DIR" ]; then
  if [ -d "$ROOT_DIR/control-plane-app" ]; then
    CORE_DIR="$ROOT_DIR"
...
if [ -n "$CORE_DIR" ] && [ -d "$CORE_DIR" ]; then
  ...
  contains "$CORE_DIR/execution-control/src/main/java/com/opensocket/aievent/core/dispatch/DispatchProperties.java" '/internal/delivery/agents/\{agentId\}/commands'
  contains "$CORE_DIR/execution-control/src/main/java/com/opensocket/aievent/core/dispatch/DispatchProperties.java" 'X-Cluster-Token'
  contains "$CORE_DIR/control-plane-app/src/main/java/com/opensocket/aievent/core/security/CoreInternalSecurityRequestClassifier.java" 'GATEWAY'
fi
