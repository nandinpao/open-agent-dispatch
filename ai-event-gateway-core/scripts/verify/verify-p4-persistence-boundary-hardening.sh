...
test "$(find "$PERSISTENCE_ROOT" -path '*/repository/Mybatis*Repository.java' | wc -l | tr -d ' ')" = "23"
test "$(find "$PERSISTENCE_ROOT" -path '*/converter/*PersistenceConverter.java' | wc -l | tr -d ' ')" = "23"
...
if grep -RIn --include='*.java' -E \
  ...
  */src/main/java/com/opensocket/aievent/core; then
...
pom=Path('database-platform/pom.xml')
...
from pathlib import Path
root=Path('.')
java=root/'database-platform/src/main/java'
...
test ! -f control-plane-app/src/main/java/com/opensocket/aievent/core/store/CoreStoreMode.java
...
if grep -RqE 'DATABASE_PLATFORM_(MAPPER|MIGRATION)_LOCATIONS' \
  control-plane-app/src/main/resources deploy/env \
