...
curl -sS -X POST "${BASE_URL}/internal/control-plane/tasks/${TASK_ID}/ack" \
  -H 'Content-Type: application/json' \
  -d "{\"dispatchRequestId\":\"${DISPATCH_REQUEST_ID}\",\"agentId\":\"${AGENT_ID}\",\"ownerGatewayNodeId\":\"${OWNER_GATEWAY_NODE_ID}\",\"agentSessionId\":\"${AGENT_SESSION_ID}\",\"attemptNo\":${ATTEMPT_NO},\"dispatchToken\":\"${DISPATCH_TOKEN}\",\"message\":\"Agent acknowledged task\"}"
echo
