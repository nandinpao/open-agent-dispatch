...
curl -sS -X POST "${BASE_URL}/internal/control-plane/tasks/${TASK_ID}/result" \
  ...
  -d "{\"dispatchRequestId\":\"${DISPATCH_REQUEST_ID}\",\"agentId\":\"${AGENT_ID}\",\"ownerGatewayNodeId\":\"${OWNER_GATEWAY_NODE_ID}\",\"agentSessionId\":\"${AGENT_SESSION_ID}\",\"attemptNo\":${ATTEMPT_NO},\"dispatchToken\":\"${DISPATCH_TOKEN}\",\"resultStatus\":\"SUCCESS\",\"message\":\"Agent completed task\",\"payload\":{\"summary\":\"mock result\"}}"
echo
