...
class R3TenantInvitationRouteProjectionTest {
    private static final Instant NOW = Instant.parse("2026-08-06T13:00:00Z");

    @Test
    void rootInvitationRouteProjectsBeforeTenantAuthorization() {
        ...
        IamSecurityAdapter adapter = new IamSecurityAdapter(request -> {
            ...
            return new AuthorizationDecision(
                    "decision-invitation", AuthorizationDecision.Effect.ALLOW,
        ...
        adapter.authorize(
                projected, resolved.rule().permission(), resolved.rule().resourceType(),
                resolved.resourceId(), resolved.rule().scopeType(),
                projected.activeTenant().tenantId(), Map.of("routePattern", resolved.rule().routeTemplate()));

        assertEquals(TenantRef.tenant("tenant-a"), projected.activeTenant());
        assertEquals("tenant-a", captured.get().requestedScopeId());
        assertEquals("user-a", captured.get().resourceId());
        assertEquals("identity.user.read", captured.get().permission());
    }
}
