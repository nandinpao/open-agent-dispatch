...
```text
control-plane-app            control plane and database owner
adapter-worker-app  stateless MCP / issue action worker
```
...
docker build \
  -f control-plane-app/Dockerfile \
  -t ai-event-gateway-core:local .

docker build \
  -f adapter-worker-app/Dockerfile \
