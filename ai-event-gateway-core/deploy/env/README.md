...
- `EVENT_DEDUP_STORE=MYBATIS` is the default for local/dev/prod, so PostgreSQL `event_dedup_state` is the source of truth. `EVENT_DEDUP_CACHE_STORE=REDISSON` keeps Redis/Redisson as an after-commit cache only.
...
- Redis single: `127.0.0.1:6379`
- Redis password: `123456`

For docker compose local usage, override `REDIS_SINGLE_ADDRESS=redis:6379` when the Core container resolves Redis by Compose service name, and start the bundled Redis with `--requirepass 123456`. Host-local defaults now stay on loopback to avoid accidental external Redis access.
